...
static void handleSend() {
  ...
  // PARAMS update allowed anytime (compact frame)
  if (mode == "Params") {
    // All are optional; we use defaults if missing
    uint16_t base = toU16(server.arg("base_speed"), 20);
    uint16_t kp   = toU16(server.arg("kp"), 2);
    uint16_t kd   = toU16(server.arg("kd"), 14);
    uint16_t tu   = toU16(server.arg("time_uturn"), 2000);
    uint16_t tt   = toU16(server.arg("time_turn"), 2000);
    uint16_t sb   = toU16(server.arg("time_strght_bfr_turn"), 3000);
    uint16_t te   = toU16(server.arg("time_turn_execute"), 6000);
    uint16_t ts   = toU16(server.arg("turnSpeed"), 20);
