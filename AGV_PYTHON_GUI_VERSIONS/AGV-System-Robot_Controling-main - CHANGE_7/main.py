...
subprocess.Popen([sys.executable, os.path.join(os.path.dirname(__file__), "agv_logs_window.py")])
...
class MainApp(ctk.CTk):
    ...
    def show_emergency_page(self):
        self.clear_frame()
        self.current_frame = EmergencyPage(self)
        self.current_frame.pack(fill="both", expand=True)

    def show_parameter_page(self):
        self.clear_frame()
        self.current_frame = ParameterPage(self)
        self.current_frame.pack(fill="both", expand=True)
