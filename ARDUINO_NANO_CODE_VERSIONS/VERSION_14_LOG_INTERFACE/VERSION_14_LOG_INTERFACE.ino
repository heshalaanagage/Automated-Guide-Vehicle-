 ...
 *  - FIRST command start (loop=0): NO U-turn, NO stopMotors
 *    -> gripDisengage -> wait -> gripEngage -> start moving
 *  - Charge/C legs: NO gripper actions (handled in Turns.ino)
 *  - No changes to T-junction functions or path flow
 ************************************************************/
...
static void applyCommand(uint8_t cmd, const uint8_t* pl, uint8_t len) {
  ...
  if (cmd == CMD_AUTO_START && len == 4) {
    ...
    // ✅ First start rule: disengage-wait-engage (NO UTURN)
  ...
  if (cmd == CMD_MANUAL_CHARGE && len == 2) {
    ...
    // ✅ No firstStart sequence for charge mode
    firstStartPending = false;
...
void loop() {
  ...
  if (agv_running && mission != MODE_NONE) {
    ...
    // ✅ FIRST START (Auto / Manual Trip only): NO STOP, NO UTURN
