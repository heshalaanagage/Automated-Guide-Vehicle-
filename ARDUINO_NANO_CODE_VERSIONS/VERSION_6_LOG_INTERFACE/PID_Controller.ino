void PID_Controller(int base_speed, int p, int d) {
  ...
  LOG_THROTTLED("PID", "pos=" + String(line_position) + " err=" + String(error) + " der=" + String(derivative) +
                        " L=" + String(left_motor_correction) + " R=" + String(right_motor_correction));
...
void checkSpecialPatterns() {
  ...
  if (bitSensor == PATTERN_T && !T_detected) {
    T_detected = true;
    LOG("DECIDE", "T_JUNCTION detected (PATTERN_T)");
    delay(300);
    LOG("FUNC", "handleTJunction()");
    handleTJunction();
  }
  ...
  if (expected && !destination_detected) {
    ...
    String d = isPick ? "PICK" : (isDrop ? "DROP" : (isCharge ? "CHARGE" : "UNKNOWN"));
    LOG("DECIDE", "DESTINATION expected -> " + d);
    LOG("FUNC", "handleDestination()");
    handleDestination();
  }
