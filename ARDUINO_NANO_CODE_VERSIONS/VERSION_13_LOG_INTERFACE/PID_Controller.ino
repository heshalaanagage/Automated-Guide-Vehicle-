...
void PID_Controller(int base_speed_in, int p, int d) {
  ...
  if (sumOnSensor > 0) {
    ...
    // Line not detected; recovery handler will manage it
    error = 0;
  }

  // Direction hint + history update (lightweight)
  char inst = 'S';

  switch (bitSensor) {
    // LEFT edge (your tuned patterns)
    case 0b11000000:
    case 0b11100000:
      direction = "left";
      inst = 'L';
    ...
    // RIGHT edge (your tuned patterns)
    case 0b00000011:
    case 0b00000111:
      direction = "right";
      inst = 'R';
  ...
  // ✅ store last non-straight direction
  if (inst == 'L' || inst == 'R') lastDir = inst;
...
static const unsigned long LINE_LOST_HOLD_MS = 3000;
