/************************************************************
 *  AGV MAIN CONTROL - VERSION 10 (COMPACT + LOCK + RESET)
 *  ESP32 -> Nano : binary frames (NO CHANGE)
 *  Logs : JSON only (for Python live log window)
 *  Short tokens: TURN=L/R/S, PATH=P-A etc
 ************************************************************/
...
// NOTE: LOGF is kept for rare logs only (MODE updates etc)
...
// NEW: ultra-light log (no heap Strings)
...
static void applyCommand(uint8_t cmd, const uint8_t* pl, uint8_t len) {
  ...
  if (cmd == CMD_EMG_STOP) {
    ...
    emergency_stop = true;
    stopMotors();
    LOGC("STATE", "E1"); // emergency stop ON
    return;
  }
  if (cmd == CMD_EMG_START) {
    emergency_stop = false;
    agv_running = (mission != MODE_NONE);
    LOGC("STATE", "E0"); // emergency stop OFF
    return;
  }
  ...
  if (emergency_stop) {
    LOGC("STATE", "IGN"); // ignored while emergency
    return;
  }
  ...
  if (cmd == CMD_MANUAL_TRIP && len == 5) {
    ...
    // drop char logged as ASCII (A/B/C) — ok
...
// This name is kept because Turns.ino calls it
